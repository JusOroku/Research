    library(rmongodb)
    library(pracma)
    create_gems_matrix <- function(mongo_object, normalize = FALSE) {
      ...
      gem_description_normalize = lapply(gem_description, function(x) (x-min(gem_description))/(max(gem_description)-min(gem_description)))
      gem_size = sapply(gem_size, as.numeric) 
      gem_size_normalize = lapply(gem_size, function(x) (x-min(gem_size))/(max(gem_size)-min(gem_size)))
      ...
      gem_authors_normalize = lapply(gem_authors, function(x) (x-min(gem_authors))/(max(gem_authors)-min(gem_authors)))
      gem_owners = sapply(gem_owners, as.numeric)
      gem_owners_normalize = lapply(gem_owners, function(x) (x-min(gem_owners))/(max(gem_owners)-min(gem_owners)))
      gem_dependencies = sapply(gem_dependencies, as.numeric)
      gem_dependencies_normalize = lapply(gem_dependencies, function(x) (x-min(gem_dependencies))/(max(gem_dependencies)-min(gem_dependencies)))
        
    #combine all of gem attribute and create matrix using name as a  row name
    if(normalize == TRUE) {
      gem_list = cbind(gem_description_normalize, gem_size_normalize, gem_license,gem_required_ruby,gem_homepage,gem_documentation,gem_source_code,gem_bug_tracker,gem_authors_normalize,gem_owners_normalize,gem_dependencies_normalize)
    }
    else {
    ...
    if(mongo.is.connected(mongo_connection) == TRUE) {
      ...
      gem_matrix_normalize = create_gems_matrix(pop, TRUE)
      normalize_distance_matrix = calculate_distance_matrix(gem_matrix_normalize)
      rownames(normalize_distance_matrix) <- rownames(gem_matrix_normalize)
    } else {
